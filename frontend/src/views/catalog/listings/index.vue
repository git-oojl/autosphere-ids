  ...
  <div class="listados">
    ...
    <div class="main-layout">
      ...
      <aside class="filters-sidebar">
        ...
        <button class="clear-filters" @click="clearAllFilters">Limpiar todo</button>
        ...
        <div class="filter-section">
          <div class="filter-header" @click="toggleSection('year')">
            <h4>Año</h4>
            <span class="toggle-icon">{{ expandedSections.year ? '−' : '+' }}</span>
          </div>
        ...
        <div class="filter-section">
          <div class="filter-header" @click="toggleSection('brand')">
            <h4>Marca</h4>
            <span class="toggle-icon">{{ expandedSections.brand ? '−' : '+' }}</span>
          </div>
          <div class="filter-content" v-show="expandedSections.brand">
            ...
            <div class="brand-list">
              <div class="filter-option" v-for="brandOption in filteredBrands" :key="brandOption">
                <label>
                  <input 
                    type="checkbox" 
                    :value="brandOption" 
                  ...
                  />
                  <span>{{ brandOption }} ({{ getBrandCount(brandOption) }})</span>
                </label>
              </div>
            </div>
          </div>
        ...
        <div class="filter-section">
          <div class="filter-header" @click="toggleSection('city')">
            <h4>Ciudad</h4>
            <span class="toggle-icon">{{ expandedSections.city ? '−' : '+' }}</span>
          </div>
      ...
      <section class="content">
        <div class="content-header">
          <h2 class="subtitle">Lista de autos ({{ filteredVehicles.length }} vehículos)</h2>
          
          <!-- SORTING DROPDOWN -->
          <select v-model="sortMethod" class="mobile-sort" @change="handleSortChange">
        ...
        <div class="active-filters" v-if="hasActiveFilters">
          <span class="active-filter" v-for="filter in activeFilters" :key="filter">
            {{ filter }}
...
const vehiclesData = ref({
  ...
  "items": [
    ...
    },
    {
      ...
      "cityId": "mx-mer",
      "sellerId": "u-seller-002",
      "coverImage": "https://images.unsplash.com/photo-1504215680853-026ed2a45def",
      "status": "draft",
      "color": "Plata"
    },
    {
...
const activeFilters = computed(() => {
  ...
  if (selectedBrands.value.length) filters.push(`${selectedBrands.value.length} marcas seleccionadas`)
  if (selectedTypes.value.length) filters.push(`${selectedTypes.value.length} tipos seleccionados`)
...
const goToVehicleDetail = (vehicleId) => {
  ...
  router.push({
    name: 'listadoDetalle', // Asegúrate que esta ruta exista en tu router
    params: { id: vehicleId }
  })
}
...
const filteredVehicles = computed(() => {
  ...
  if (selectedCities.value.length > 0) {
    result = result.filter(v => selectedCities.value.includes(getCityName(v.cityId)))
  }
