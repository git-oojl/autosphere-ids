<template>
  <section class="saved-page">
    <header class="hero">
      ...
      <p>
        Tu lista guardada ya existe como superficie real, con salto a detalle y
      ...
      <article v-for="item in items" :key="item.id" class="card vehicle-card">
        ...
        <div class="content">
          ...
          <div class="actions">
            ...
            <button class="secondary" @click="openSeller(item.sellerId)">
              Ver anunciante
            </button>
          </div>
        </div>
...
function openSeller(id) {
  if (id) router.push({ name: 'public-user-profile', params: { id } });
}
onMounted(async () => {
  const response = await getSavedVehicles();
